/** Home screen — two states (new player / returning) per docs/design/home.jsx.
 * Title stays "ROBOT TOWER" (the original app wins on names; the mock's
 * "Core Defender" is a placeholder unless renamed deliberately). */
  ...
  render(): void {
    ...
    const strip = !returning ? "" : `
      <div class="home-strip">
        <div class="hs-box"><label>REACHED</label><b class="lt">Level ${gs.level}</b></div>
        <div class="hs-box"><label>CORES</label><span class="hs-val"><span class="core-icon small"></span><b class="cy">${gs.cores}</b></span></div>
        <div class="hs-box"><label>TOWER LVL</label><b class="cy">${gs.towerLevel}</b></div>
        <div class="hs-box hs-click" data-act="ach"><label>SKILLS</label><b class="gr">${gs.skills.size} / ${SKILL_NODES.length}</b></div>
      </div>`;
      ...
      <div class="home-menu">
        ...
        <div class="menu-row">
          <button class="menu-btn green" data-act="skills"><span class="mb-icon">⏣</span>
            <span class="mb-col"><span class="mb-title">SKILL TREE</span><span class="mb-sub">Spend cores</span></span></button>
    ...
    this.root.querySelector("[data-act=ach]")?.addEventListener("click", () => this.onSkills());
  }
}
